...
cs_rels_from_a7_virtual_SFVG_to_Farm - This script extracts the Server For Virtual Guests to ESX Cluster Relations knowledge from Assetcenter.
...
This script extracts the ComputerSystem virtual server information from Assetcenter. It uses the Asset Centre relations extract to build the relations between the Server For Virtual Guests (ESX Servers) and Farms (Virtual Clusters). The results are loaded int the CIM database.


=head1 SYNOPSIS

 cs_rels_from_a7_virtual_SFVG_to_Farm.pl [-t] [-l log_dir] [-c]

 cs_rels_from_a7_virtual_SFVG_to_Farm.pl -h    Usage
 cs_rels_from_a7_virtual_SFVG_to_Farm.pl -h 1  Usage and description of the options
 cs_rels_from_a7_virtual_SFVG_to_Farm.pl -h 2  All documentation
...
$summary_log->info("Getting Server For Virtual Guest to Farms relations");
...
# Initialize relation
my $relation = "Is Physical Server For";
...
SELECT `Asset tag (*_ Local CI)`, `*_ Hostname / inst (*_ Local CI)`, `* Logical CI type (*_ Local CI)`, `* Oper System`,
  ...
  WHERE `*_ Relation type` = 'Logical Server'
    AND `* Logical CI type (*_ Local CI)`= 'Logical Server'
    AND `* Logical CI type (*_ Distant CI)` IS NULL
    AND `*_ Impact direction` = 'Local impacts Distant'
    AND `* Oper System` in ('VMWARE ESX', 'ESX Cluster')") or exit_application(2);
...
while (my $ref = $sth->fetchrow_hashref) {
        ...
        # Set local_name to Farm
        $source_system_element_id = $ref->{'Asset tag (*_ Local CI)'} || "";
        set_virtual_info($dbt, $source_system_element_id, "Farm", $virtualtechno) or exit_application(2);
