...
cs_rels_from_a7_virtual_VG_to_Farm - This script extracts the Virtual Guests to ESX Cluster Relations knowledge from Assetcenter.
...
This script extracts the ComputerSystem virtual server information from Assetcenter. It uses the Asset Centre relations extract to build the relations between the Virtual Guests (Virtual machines) and Farms (Virtual Clusters). The results are loaded int the CIM database.


=head1 SYNOPSIS

 cs_rels_from_a7_virtual_VG_to_Farm.pl [-t] [-l log_dir] [-c]

 cs_rels_from_a7_virtual_VG_to_Farm.pl -h    Usage
 cs_rels_from_a7_virtual_VG_to_Farm.pl -h 1  Usage and description of the options
 cs_rels_from_a7_virtual_VG_to_Farm.pl -h 2  All documentation
...
$summary_log->info("Getting Virtual Guest to Farms relations");
