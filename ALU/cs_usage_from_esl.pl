...
cs_usage_from_esl - This script will extract the ComputerSystem Usage Information from ESL.
...
=head1 SYNOPSIS
 ...
 cs_usage_from_esl.pl [-t] [-l log_dir] [-c]

 cs_usage_from_esl.pl -h    Usage
 cs_usage_from_esl.pl -h 1  Usage and description of the options
 cs_usage_from_esl.pl -h 2  All documentation
...
$summary_log->info("Processing data for Usage - Get Query");

my $sth = do_execute($dbs, "
SELECT `Usage`, `Detailed Usage`, `System ID`
  FROM esl_cs_usage
  WHERE `Usage` IS NOT NULL") or exit_application(2);

$summary_log->info("Processing data for Usage - Process data");
...
while (my $ref = $sth->fetchrow_hashref) {
        ...
        # Usage
        my ($system_usage_id);
        my $system_service_usage_code = $ref->{"Usage"} || "Review System Usage";
        my $system_usage_details = $ref->{"Detailed Usage"} || "";
        @fields = ("computersystem_id", "system_service_usage_code", "system_usage_details");
        (@vals) = map { eval ("\$" . $_ ) } @fields;
        if ( val_available(\@vals) eq "Yes") {
                $system_usage_id = create_record($dbt, "system_usage", \@fields, \@vals) or exit_application(2);
        } else {
                $system_usage_id = undef;
        }
