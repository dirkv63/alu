...
Load of master XLS sheets in the alu_cmdb database.
- alu_workgroups
- alu_pf_security
- alu_acronym_mapping
...
use WorkFlow qw(read_master_SOURCE);
...
Load incoming master file. RUN is the symbolic name for the specific run, that can be found
...
$summary_log->info("Importing Master files in the $database database");
...
my $source_info = read_master_SOURCE();
...
foreach my $source_item (@$source_info) {
    ...
    # File is a file name + tab name (eg. master_workgroup.xls|Alternative 2)
    # Keep processing identical to A7 and treat names like globs (not used for master)
