...
Laden van ovsd csv files in de alu_cmdb database.
...
use WorkFlow qw(read_ovsd_SOURCE);
...
Load incoming ovsd files. RUN is the symbolic name for the specific run, that can be found
...
$summary_log->info("Importing ovsd files in the $database database");
...
foreach my $ovsd_item (@$ovsd_info) {
    my ($table, $file, $charset, $expected_row_count) = map { $ovsd_item->{$_} } qw(Table File CharSet RowCount);

    # File is a file pattern + tab name (eg. Database CI Report on *.xls|DB_CI_Relationships)

    my ($xls, $tab) = split(/\|/, $file);

    # filtering is done with a glob.
    if ($opt_file) {
        my $re = glob2pat($opt_file);
        next unless ($xls =~ qr/$re/);
    }

    # sort files|sheet per excel file (so we need to parse it only once)
    $wanted_tables->{$xls}->{$tab} = [ $table, $charset, $expected_row_count ];
}
...
foreach my $xls (keys %$wanted_tables) {
    # xls is a shell wildcard pattern (glob it)

    my $file_pattern = File::Spec->catfile($ds_step, $xls);
    ...
    foreach my $tab (keys %{$wanted_tables->{$xls}}) {
        ...
        my ($table, $charset, $expected_row_count) = @{ $wanted_tables->{$xls}->{$tab} };
