...
locations_from_a7 - Extract Location information from A7.
...
This script will extract location information from Assetcenter. This will be done in some steps.
...
=head1 SYNOPSIS
 ...
 locations_from_a7.pl [-t] [-l log_dir] [-c]

 locations_from_a7.pl -h    Usage
 locations_from_a7.pl -h 1  Usage and description of the options
 locations_from_a7.pl -h 2  All documentation
...
while (my $ref = $sth->fetchrow_hashref) {
        ...
        if (length($location_id) == 0) {
                ...
                if ( val_available(\@vals) eq "Yes") {
                        ...
                        $data_log->error("Trying to create location record, but no data available. Exiting...");
                }
        }
}
...
$sth = do_execute($dbs, "
SELECT `*_ Hostname / inst`, `* IP domain`, `* Office`, `Full name (* Location)`, `Asset tag`
  FROM a7_servers
  WHERE `Country (* Location)` IS NOT NULL") or exit_application(2);
...
while (my $ref = $sth->fetchrow_hashref) {
        ...
        my $object_name = lc($hostname) . "." . lc($domainname);
        my $floor_slot = $ref->{"* Office"} || "";
        my $location_code = $ref->{"Full name (* Location)"} || "";
        my @fields = ("location_code");
        my (@vals) = map { eval ("\$" . $_ ) } @fields;
        my $location_id;
