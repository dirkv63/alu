...
I will make an ALU specific ini-file subroutine. Use this routine to get the LOGDIR parameter from
the current section or from the default section, and pass this parameter to the setup_logging subroutine.
...
sub setup_logging {
  ...
  my $ini_logdir;
  if (defined $alu_cfg) {
    if ($alu_cfg->SectionExists($INI_SECTION)) {
      if ($alu_cfg->val($INI_SECTION, 'LOGDIR')) {
        $ini_logdir = $alu_cfg->val($INI_SECTION, 'LOGDIR');
      }
    }
    else {
  ...
  my $logdir = (defined $ini_logdir) ? $ini_logdir : 'd:\temp\log';
